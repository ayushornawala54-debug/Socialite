...
if (isset($_POST['update'])) {
	...
	$group_image = $conn->real_escape_string($_POST['group_image']);
    ...
    $conn->query("UPDATE user_profiles 
                  SET group_name='$group_name',group_description='$group_description',group_image='$group_image', created_by='$created_by',created_at='$created_at'
                  WHERE id=$id");
    header("Location: groupmember.php");
    exit();
}
...
// Fetch Data
$result = $conn->query("SELECT id, group_name,  group_description, group_image, created_by, created_at  FROM tbl_groups");
?>
    ...
    <?php if ($result->num_rows > 0) : ?>
        <?php while ($row = $result->fetch_assoc()) : ?>
            <tr>
                ...
                <td><?= $row['group_image']; ?></td>
                <td><?= $row['created_by']; ?></td>
                <td><?= $row['created_at']; ?></td>
                <td>
                    <a href="userp.php?delete_id=<?= $row['id']; ?>" onclick="return confirm('Are you sure?')">Delete</a> |
