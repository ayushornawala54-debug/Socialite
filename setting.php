...
if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] == 0) {
    ...
    if (move_uploaded_file($_FILES["profile_photo"]["tmp_name"], $target_file)) {
        $updateQuery = "UPDATE tbl_register SET profile_photo = ? WHERE id = ?";
...
$result = $conn->query("SELECT * FROM user_profiles WHERE user_id = '$user_id'");
$existingData = $result->fetch_assoc();

if (isset($_POST['submit'])) {
    $username = !empty($_POST['username']) ? $conn->real_escape_string($_POST['username']) : $existingData['username'];
    $email = !empty($_POST['email']) ? $conn->real_escape_string($_POST['email']) : $existingData['email'];
    $bio = !empty($_POST['bio']) ? $conn->real_escape_string($_POST['bio']) : $existingData['bio'];
    $gender = !empty($_POST['gender']) ? $conn->real_escape_string($_POST['gender']) : $existingData['gender'];
    $relationship = !empty($_POST['relationship']) ? $conn->real_escape_string($_POST['relationship']) : $existingData['relationship'];

    if ($existingData) {
        $sql = "UPDATE user_profiles 
                SET username = '$username', email = '$email', bio = '$bio', gender = '$gender', relationship = '$relationship' 
                WHERE user_id = '$user_id'";
    } else {
        $sql = "INSERT INTO user_profiles (user_id, username, email, bio, gender, relationship) 
                VALUES ('$user_id', '$username', '$email', '$bio', '$gender', '$relationship')";
    }

    if ($conn->query($sql) === TRUE) {
        $updateUser = "UPDATE tbl_register SET username = '$username', email = '$email' WHERE id = '$user_id'";
        ...
        echo "❌ Error inserting/updating profile: " . $conn->error;
    }
}
...
if (!isset($_SESSION['user_email'])) {
    echo "❌ Error: No session email found.";
    exit();
}

$email = trim(strtolower($_SESSION['user_email']));

$stmt = $conn->prepare("SELECT username, bio, email FROM user_profiles WHERE TRIM(LOWER(email)) = ?");
...
if ($result->num_rows > 0) {
    ...
    echo "✅ Profile Found: " . htmlspecialchars($user['username']);
} else {
...
<body>
                            ...
                            <div  class="hidden bg-white rounded-lg drop-shadow-xl dark:bg-slate-700 w-64 border2"
                                ...
                                <a href="timeline.php">
                                    <div class="p-4 py-5 flex items-center gap-4">
                                        ...
                                        <form method="POST">
											<div class="flex-1">
											<img src="/uploads/stories/<?php echo htmlspecialchars(basename($profile_photo)); ?>" alt="Profile Photo" class="w-10 h-10 rounded-full shadow">
    ...
    <label for="file" class="cursor-pointer">
        ...
        <img id="imgPreview" src="<?php echo htmlspecialchars($profile_photo, ENT_QUOTES, 'UTF-8'); ?>" 
     class="object-cover w-full h-full rounded-full" alt="Profile Photo">

    </label>
    
    <!-- Hidden file input -->
    <input type="file" name="profile_photo" id="file" class="hidden" accept="image/*" onchange="previewImage(event)">

    <!-- Update button (Initially disabled) -->
    <button id="updateBtn" class="bg-blue-500 text-white px-4 py-2 rounded mt-2 hidden" onclick="uploadImage()">Update</button>
</div>
...
<script>
    function previewImage(event) {
        ...
        reader.onload = function(){
            ...
            document.getElementById('updateBtn').classList.remove('hidden'); // Show update button
        }
    ...
    function uploadImage() {
        ...
        .then(data => {
            ...
            location.reload(); // Refresh to update the displayed profile image
        })
...
<form method="POST">
    <div class="flex-1">
        ...
        <button type="submit">Update</button>
                        ...
                        <button class="inline-flex items-center gap-1 py-1 pl-2.5 pr-3 rounded-full bg-slate-50 border-2 border-slate-100 dark:text-white dark:bg-slate-700" type="button" aria-haspopup="true" aria-expanded="false"> 
                           ...
                           <span class="font-medium text-sm"><a href="upgrade.php"> Upgrade  </a></span> 
                        </button>
                    </div>
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="md:flex items-center gap-10">
                                        ...
                                        <div class="flex-1 max-md:mt-4">
                                            <input type="text" name="username" placeholder="Monroe" class="lg:w-1/2 w-full">
                                        </div>
                                    </div>
                                    ...
                                    <div class="md:flex items-center gap-10">
                                        ...
                                        <div class="flex-1 max-md:mt-4">
                                            <input type="text" name="email" placeholder="info@mydomain.com" class="w-full">
                                        </div>
                                    </div> 
                                    ...
                                    <div class="md:flex items-start gap-10">
                                        ...
                                        <div class="flex-1 max-md:mt-4">
                                            <textarea name="bio" class="w-full" rows="5" placeholder="Inter your Bio"></textarea>
                                        </div>
                                    </div> 
                                    ...
                                    <div class="md:flex items-start gap-10 " hidden>
                                        ...
                                        <div class="flex-1 flex items-center gap-5 max-md:mt-4">
                                            <img src="./socialminds/assets/images/avatars/avatar-3.jpg" alt="" class="w-10 h-10 rounded-full">
                                ...
                                <div class="flex items-center gap-4 mt-16 lg:pl-[10.5rem]">
                                    <button type="submit" name="reset" class="button lg:px-6 bg-secondery max-md:flex-1"> Cancel</button>
                                    <button type="submit" name="submit" class="button lg:px-10 bg-primary text-white max-md:flex-1"> Save <span class="ripple-overlay"></span></button>
                                </div>
						...
						<form method="POST">
                            ...
                            <div class="max-w-md mx-auto">
                                ...
                                <div class="font-normal text-gray-400">
                                    ...
                                    <div class="space-y-6 mt-8">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="link" class="w-full" placeholder="http://www.facebook.com/myname">
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="link" class="w-full" placeholder="http://www.instagram.com/myname">
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="link" class="w-full" placeholder="http://www.twitter.com/myname">
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="link" class="w-full" placeholder="http://www.youtube.com/myname">
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="link" class="w-full" placeholder="http://www.github.com/myname">
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center justify-center gap-4 mt-16">
                                    ...
                                    <button type="submit" name="submit" class="button lg:px-10 bg-primary text-white max-md:flex-1"> Save</button>
                                </div>
    ...
    <svg class="w-6 h-6 group-aria-expanded:hidden duration-500" xmlns="http://www.w3.org/2000/svg" fill="none"
        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
            d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
    </svg>

    <svg class="w-6 h-6 -mr-1 hidden group-aria-expanded:block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
        fill="currentColor">
